...
**2️⃣ 해결 과정** <br>
  - 환경 변수 기반으로 yml 수정<br>
    : `application.yml` 내 민감한 값들을 모두 환경 변수 참조로 변경 (로컬 실행 시 기본값(default value)을 함께 지정해 Jenkins/로컬 겸용으로 구성)
  - Secret manifest 생성 (backend-secret.yaml)
  - `kubeseal` 설치 및 암호화 수행
  - SealedSecret 적용 (kubectl apply -f backend-sealedsecret.yaml)<br>
    → `sealed-secrets-controller`가 자동으로 `backend-secret` 복호화하여 생성<br>
        
**3️⃣ 결과**<br>
  - 민감한 정보를 환경변수 + Secret주입으로 더 안전하게 저장
  - Secret관리를 kubeseal로 자동 암호화
...
| 조원 이름	| 회고  	 |
| -----	|-------|
|   이진구   |  CI/CD는 처음 접해보는 경험이라 부트캠프 중에서 이해 난이도가 가장 어려웠던 것 같습니다. 그래도 수업을 열심히 따라가면서 하나씩 프로젝트에 적용이 되는 것을 보고 희열을 느꼈습니다. 지금까지 설계부터 개발, 그리고 배포까지 배우면서 많은 지식을 얻은 것 같고 의미있는 시간이 되었습니다. 개발자로서의 계단을 한 단계 오른 것 같고 다음에는 로컬뿐만 아니라 실제로 배포를 하는 것을 목표로 하겠습니다. |
|   박진우   | 단순하게 통합해주는거 배포해주는게 CICD인줄 알았는데 생각보다 어려웠다. 각각의 프로젝트 상황에 맞게 CI/CD의 규모를 고민하는 연습을 할 수 있는 좋은 기회였다. 취업을 한다면 어떻게 돌아가는지 이해할 수 있는게 가장 큰 배움이였다.  |
|   윤석현   | CI/CD를 경험하면서 사람이 수작업으로 할때 발생할 수 있는 실수들을 툴들을 활용하여 줄일 수 있다는 점이 가장 매력적으로 다가왔다. 상황에 맞는 CI/CD툴을 선택하는게 좋을 것 같다는 생각이 들었고, 더 많은 툴들을 찾아볼 수 있는 계기가 되었다. |
|   조상원   | 이번 프로젝트를 통해 CICD를 처음으로 경험하면서, 이론적으로만 배웠던 개념들이 실제로 어떻게 동작하는지 직접 체감할 수 있었다. 처음에는 개념을 이해하는 데에도 많은 시간이 걸렸지만, 직접 적용해보며 자동화된 빌드 및 배포 과정의 흐름을 몸으로 익힐 수 있었다. 이전까지는 단순 개발 단계에서 끝났다면, 이번에는 DevOps 환경을 구축하여 무중단 배포까지 구현해보며 개발뿐만 아니라 운영과 배포까지 아우르는 전 과정의 중요성을 느낄 수 있었다. 또한, 서비스의 안정성과 효율성을 높이는 방법을 실감했다. 이번 학습을 바탕으로 앞으로는 더 체계적이고 자동화된 개발 환경을 구축해 나갈 것이다. |
|   최유경 	 |  CI/CD가 뭔지도 잘 몰랐고 처음해보는 배포 과정이라 어려움이 많았다. 그래도 이론으로만 배웠을때보다 직접 프로젝트를 진행하며 kubernetes, jenkins, argocd 작동과정에 대해서 더 이해할 수 있었고 전체적인 흐름을 알 수 있었다. 그리고 단순 개발에서 끝나는게 아니라 배포 과정까지 경험하며 하나의 서비스가 완성되기까지 정말 많은 노력이 필요하다는 것과 자동 배포 과정의 중요성을 느낄 수 있었습니다.  |
